...
class Multi_Head_Attention(tf.keras.layers.Layer):
    ...
    def call(self, v, k, q, mask):
       ...
       attention_all_heads = tf.concat(attention_weights_all, axis = 0)
       return output,attention_weights,attention_all_heads
...
class Encoder_layer(tf.keras.layers.Layer):
    ...
    def call(self, x_v, x_k, x_q, mask, training):
        headAttention, encoder_weight,attention_all_heads = self.attentionLayer(x_v, x_k, x_q, mask)
        ...
        return outputSubLayer2, encoder_weight,attention_all_heads
...
class Transformer_Encoder(tf.keras.layers.Layer):
    ...
    def call(self, x, training = True):
        ...
        all_outputs = []
        all_weights = []  
        all_weights_heads_layers = []
        for i in range(self.num_encoders):
            x,encoder_weights,weights_all_heads = self.encoderStack[i](x, x, x, mask_smiles, training) # v, k, q
            all_weights.append(encoder_weights)
            all_outputs.append(x)
            all_weights_heads_layers.append(weights_all_heads)

        return x, all_weights, all_outputs,all_weights_heads_layers  # (batch_size, input_seq_len, d_model)
...
class Masked_Smiles_Model(tf.keras.Model):#(tf.keras.layers.Layer):#(tf.keras.Model):
    ...
    def call(self, x, training = True):     
        x, all_weights, all_outputs,all_weights_heads_layers = self.transformer_encoder(x, training)      
        x_ff = self.ffnn(x)
        return x, x_ff, all_weights,all_weights_heads_layers
